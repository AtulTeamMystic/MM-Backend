...
| `clanLeaderboardJob.refresh` | `us-central1` | Cloud Scheduler (`every 15 minutes`) | Writes top-100 clan leaderboard to `/Leaderboards/Clans`. |
| `leaderboards.refreshAll` | `us-central1` | Cloud Scheduler (`every 15 minutes`) | Recomputes player leaderboards (XP, trophies, etc.). |
