...
/Leaderboards_v1/{metric} (singleton docs per metric)
/Leaderboards/Clans (singleton)
...
### `/Leaderboards_v1/{metric}` and `/Leaderboards/Clans`

Two cached leaderboard families keep read costs predictable:

* `/Leaderboards_v1/{metric}` (one doc per metric such as `trophies`, `careerCoins`, `totalWins`) holds the top 100 player rows built by the scheduled job in `src/Socials/leaderboardJob.ts`. Fields:
  ...
  * `youCache` *(map)* ? `{ [uid]: { rank, value } }` so `getGlobalLeaderboard` can report the caller's rank even when they are outside the top 100 without reading any extra docs.
* `/Leaderboards/Clans` (singleton) carries the clan leaderboard snapshot generated by `src/clan/leaderboardJob.ts`. Fields:
